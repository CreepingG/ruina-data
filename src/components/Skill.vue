...
import { skills, Skill, text, attributes, states } from './Data' 
...
function Filterd(source:{'@id':any, 'name'?:any}[], oneHot:any):any[]{
  return source.filter(v=>oneHot[(v['@id'] as number)-1]);
}
function HiddenForSearch(skill:Skill){
  const filter = props.filter;
  if (!filter) return false;
  if (filter.match(/\d+/)){
    if (skill['@id'] === Number(filter)) return false;
  }
  ...
  <el-container style="height:100%;">
    <el-aside width="180px" style="padding: 0;">
      <el-menu :default-active="id.toString()" >
        <el-menu-item v-for="(item,i) in skills" :key="item['@id']" :index="item['@id'].toString()" 
          :hidden="HiddenForSearch(item)"
          @click="onclick(item['@id'] as number)" class="menu-item">
      ...
      <el-descriptions :title="(cur.name || '') + '@' + cur['@id']" :column="column" border>
        ...
        <template v-if="cur.type === 0"><!--一般-->
          ...
          <el-descriptions-item v-if="Any(cur.attribute_effects)" :label="text.attribute" :span="column">
            <el-tag v-for="attr in Filterd(attributes, cur.attribute_effects)" :key="attr['@id']" class="label">
              {{attr.name}}
            </el-tag>
          </el-descriptions-item>
          <!--状态-->
          <el-descriptions-item v-if="Any(cur.state_effects)" :label="text.state" :span="column">
            <el-tag v-for="effect in Filterd(states, cur.state_effects)" :key="effect['@id']" class="label"
